...
const COMPREHENSIVE_GUIDANCE_PART2 = {
        ...
        "AC.L2-3.1.2": {
            ...
            cloud: {
                aws: {
                    ...
                    implementation: {
                        steps: ["Create fine-grained IAM policies limiting actions per user role","Use IAM policy conditions to restrict actions based on context","Implement Service Control Policies (SCPs) to prevent dangerous actions","Use resource-based policies to restrict access to specific resources","Enable CloudTrail to log all API calls for audit","Use IAM Access Analyzer to identify overly permissive policies","Implement permission boundaries for delegated administration"],
                        policy_example: `{
  "Version": "2012-10-17",
  "Statement": [
    {
    ...
    },
    {
      ...
      "Resource": "*"
                        ...
                        verification: ["Test user can only perform allowed actions","Verify denied actions are blocked","Review CloudTrail for unauthorized attempts","Run IAM Access Analyzer to check for issues"],
  ...
  "AssignableScopes": [
    "/subscriptions/{subscription-id}/resourceGroups/cui-resources"
                        ...
                        verification: ["Test custom role permissions","Verify deny assignments block critical operations","Review Activity Log for unauthorized attempts","Check Azure Policy compliance"],
                ...
                gcp: {
                    ...
                    implementation: {
                        steps: ["Create custom IAM roles with specific permissions","Use predefined roles as starting point","Implement organization policies to restrict operations","Use VPC Service Controls for perimeter security","Enable Cloud Audit Logs for all admin activity","Use IAM Recommender to identify excessive permissions","Implement resource hierarchy for inheritance"],
...
- storage.objects.update`,
                        verification: ["Test custom role permissions: gcloud projects get-iam-policy","Verify organization policies block restricted operations","Review Cloud Audit Logs","Check IAM Recommender suggestions"],
            ...
            containers: {
                kubernetes: {
                    ...
                    implementation: {
                        steps: ["Define Roles with specific verbs (get, list, watch vs create, update, delete)","Use ClusterRoles for cluster-wide permissions","Implement admission controllers to validate requests","Use OPA Gatekeeper for policy enforcement","Enable audit logging for all API requests","Implement resource quotas to limit resource consumption"],
...
- apiGroups: ["apps"]
  ...
  # Can view but not modify deployments`,
                        verification: ["Test role permissions: kubectl auth can-i create secrets --as=user -n cui-workloads","Verify admission controllers block unauthorized operations","Review audit logs for denied requests"],
                        effort_hours: 8
                    }
                }
            },
            saas: {
                salesforce: {
                    ...
                    implementation: {
                        steps: ["Create profiles with minimal object and field permissions","Use permission sets for additive permissions","Implement field-level security to restrict sensitive fields","Use validation rules to enforce business logic","Enable Setup Audit Trail to track permission changes","Implement record-level security with sharing rules","Use permission set groups for complex scenarios"],
                        verification: ["Test user can only perform allowed operations","Verify field-level security hides sensitive data","Review Setup Audit Trail for permission changes","Check Event Monitoring for unauthorized access attempts"],
                        effort_hours: 12
                    }
                },
                microsoft365: {
                    ...
                    implementation: {
                        steps: ["Configure SharePoint permission levels (Read, Contribute, Edit)","Use Exchange RBAC for mailbox permissions","Implement Teams channel permissions","Use sensitivity labels to restrict actions on documents","Enable audit logging for all permission changes","Implement DLP policies to prevent unauthorized actions"],
                        verification: ["Test users can only perform allowed SharePoint operations","Verify Exchange permissions are correct","Review audit logs for unauthorized attempts","Check DLP policy violations"],
                        effort_hours: 10
                    }
                }
            },
            custom_app: {
                java: {
                    ...
                    implementation: {
                        steps: ["Use Spring Security method-level security annotations","Implement custom AccessDecisionVoter for fine-grained control","Use @PreAuthorize and @PostAuthorize annotations","Implement role hierarchy for permission inheritance","Log all authorization decisions","Use Spring Security ACLs for object-level security"],
...
public class SecurityConfig extends WebSecurityConfigurerAdapter {
    ...
    public RoleHierarchy roleHierarchy() {
        ...
        return hierarchy;
                        ...
                        verification: ["Test method-level security with different roles","Verify role hierarchy works correctly","Check audit logs for authorization decisions","Test @PostAuthorize filters results correctly"],
                        effort_hours: 16
                    }
                },
                dotnet: {
                    ...
                    implementation: {
                        steps: ["Implement policy-based authorization","Use [Authorize] attribute with policies","Create custom authorization handlers","Implement resource-based authorization","Log all authorization decisions","Use claims-based authorization for fine-grained control"],
...
public class CUIDataController : ControllerBase
{
    ...
    public async Task<IActionResult> DeleteData(int id)
    {
        ...
        return NoContent();
                        ...
                        verification: ["Test policy-based authorization with different users","Verify resource-based authorization works","Check logs for authorization decisions","Test custom authorization handlers"],
                        effort_hours: 14
                    }
                }
            },
            database: {
                postgresql: {
                    ...
                    implementation: {
                        steps: ["Create database roles for different access levels","Grant specific privileges (SELECT, INSERT, UPDATE, DELETE) per role","Implement Row-Level Security (RLS) policies","Use column-level privileges for sensitive fields","Enable pgAudit for detailed logging","Use SECURITY DEFINER functions for controlled operations"],
...
ALTER SYSTEM SET pgaudit.log_relation = on;`,
                        verification: ["Test role permissions: SET ROLE cui_reader; SELECT * FROM cui_data;","Verify RLS policies filter data correctly","Check pgAudit logs for all operations","Test column-level security hides sensitive fields"],
                        effort_hours: 10
                    }
                },
                mongodb: {
                    ...
                    implementation: {
                        steps: ["Create custom roles with specific privileges","Use built-in roles as templates","Implement field-level redaction for sensitive data","Enable audit logging for all database operations","Use views to restrict data access","Implement client-side field-level encryption"],
    ...
    },
    {
      $project: {
        ...
        classified_notes: 0  // Hide classified notes
                        ...
                        verification: ["Test role permissions with different users","Verify field-level redaction works","Check audit logs for all operations","Test views restrict data access"],
                        effort_hours: 12
                    }
                }
            },
            small_business: {
                ...
                implementation: {
                    steps: ["Use AWS IAM managed policies or Azure built-in roles","Avoid custom policies initially - use least privilege predefined roles","Document which roles can perform which operations","Use cloud provider's free audit logging","Implement simple approval workflow for sensitive operations","Review permissions quarterly"],
                    tools: [
            ...
            firewalls: {
                paloalto: {
                    services: ["PAN-OS","GlobalProtect","Prisma Access","User-ID"],
                    implementation: {
                        steps: ["Configure User-ID integration with AD for identity-based access policies","Implement GlobalProtect VPN with HIP checks for endpoint compliance","Create Security Policies using App-ID to restrict application access by user group","Configure zone-based access controls to segment CUI networks","Enable SSL Decryption to inspect encrypted traffic for unauthorized access","Deploy Prisma Access for consistent remote user access policies","Forward logs to SIEM for access event correlation"],
                        cost_estimate: "$3,000-15,000/year",
                        effort_hours: 12
                    }
                }
            },
            xdr_edr: {
                sentinelone: {
                    services: ["Singularity XDR","Device Control","Firewall Control","Ranger"],
                    implementation: {"steps":["Deploy SentinelOne agent on all CUI endpoints for endpoint-level access controls","Enable Device Control to block unauthorized USB/removable media on CUI systems","Configure Firewall Control to restrict network access from CUI endpoints","Use Ranger to discover unmanaged devices on CUI networks","Enable ITDR to detect credential misuse and unauthorized access","Configure STAR custom rules for CUI file access alerting"],"cost_estimate":"$5-12/endpoint/month","effort_hours":8}
                }
            },
            rmm: {
                ninjaone: {
                    services: ["NinjaOne RMM","Endpoint Monitoring","Scripting","Documentation"],
                    implementation: {"steps":["Deploy NinjaOne agent on all CUI-scope endpoints","Create dedicated CUI Organization with hardened access policies","Configure role-based technician access for CUI system management","Set up monitoring for unauthorized software and configuration changes","Use scripting to audit local accounts and group memberships on CUI endpoints","Maintain CUI asset inventory in NinjaOne Documentation"],"cost_estimate":"$3-5/endpoint/month","effort_hours":6}
                }
            },
            vuln_mgmt: {
                tenable: {"services":["Tenable.io","Nessus"],"implementation":{"steps":["Run credentialed compliance scans using CIS/DISA STIG benchmarks on CUI systems","Audit access control configurations: local accounts, group policies, permissions","Scan for default credentials and weak authentication","Tag CUI assets for isolated reporting","Schedule compliance audits to detect access control drift"],"cost_estimate":"$30-65/asset/year","effort_hours":8}}
            }
        }
        ...
        "AU.L2-3.3.1": {
            ...
            cloud: {
                aws: {
                    ...
                    implementation: {
                        steps: ["Enable CloudTrail in all regions with management and data events","Configure CloudTrail to log to S3 bucket with encryption","Enable CloudTrail log file validation","Set up CloudWatch Logs for real-time monitoring","Implement S3 lifecycle policies for log retention (7 years for CUI)","Use S3 Object Lock for WORM (Write Once Read Many) compliance","Configure Athena for log analysis","Enable VPC Flow Logs for network traffic","Enable S3 access logging","Enable RDS/Aurora audit logging"],
    ...
    rule {
      default_retention {
        ...
        days = 2555
                        ...
                        verification: ["Verify CloudTrail is logging: aws cloudtrail get-trail-status","Check log files are encrypted and validated","Test log retention policies","Query logs with Athena to verify completeness"],
...
az monitor diagnostic-settings subscription create \\
  ...
  --logs '[{"category": "Administrative", "enabled": true}, 
          ...
          {"category": "Alert", "enabled": true}]'`,
                        verification: ["Check Activity Log is collecting events","Verify logs are flowing to Log Analytics","Test log queries in Log Analytics","Verify archive storage has immutability enabled"],
                ...
                gcp: {
                    ...
                    implementation: {
                        steps: ["Enable Admin Activity audit logs (enabled by default)","Enable Data Access audit logs for CUI resources","Enable System Event audit logs","Configure log sinks to Cloud Storage for long-term retention","Use bucket lock for WORM compliance","Set up log sinks to BigQuery for analysis","Configure VPC Flow Logs","Enable Cloud DNS logging","Set retention period (7 years for CUI)"],
...
gcloud compute networks subnets update cui-subnet \\
  ...
  --logging-flow-sampling=1.0`,
                        verification: ["Check audit logs are being generated: gcloud logging read","Verify log sink is working","Test bucket retention policy","Query logs in BigQuery"],
            ...
            containers: {
                kubernetes: {
                    ...
                    implementation: {
                        steps: ["Enable Kubernetes audit logging on API server","Configure audit policy to log all requests","Send audit logs to external SIEM or log aggregator","Enable container runtime logging","Use Fluentd/Fluent Bit for log collection","Store logs in centralized location (S3, Cloud Storage, etc.)","Implement log retention (7 years for CUI)","Enable audit logging for admission controllers"],
...
--audit-log-maxsize=100`,
                        verification: ["Check audit logs are being generated: tail -f /var/log/kubernetes/audit.log","Verify audit policy is applied","Test log aggregation to SIEM","Verify log retention settings"],
                        effort_hours: 12
                    }
                }
            },
            saas: {
                microsoft365: {
                    ...
                    implementation: {
                        steps: ["Enable Unified Audit Logging for all users","Enable mailbox auditing for all mailboxes","Enable SharePoint audit logging","Configure audit log retention (7 years with E5 or add-on)","Export audit logs to SIEM for long-term storage","Enable Teams audit logging","Configure alerts for suspicious activities","Use Microsoft Purview for compliance management"],
...
Search-UnifiedAuditLog -StartDate (Get-Date).AddDays(-90) \\
  ...
  -ResultSize 5000`,
                        verification: ["Verify audit logging is enabled: Get-AdminAuditLogConfig","Check mailbox audit is enabled for all users","Test audit log search functionality","Verify logs are being exported to SIEM"],
                ...
                salesforce: {
                    ...
                    implementation: {
                        steps: ["Enable Event Monitoring (requires add-on)","Configure Setup Audit Trail retention","Enable Field Audit Trail for sensitive fields","Set up Event Log Files for download","Export event logs to external SIEM","Enable Login Forensics","Configure Transaction Security policies","Implement real-time event monitoring"],
...
// Setup > Object Manager > [Object] > Fields & Relationships > Set History Tracking`,
                        verification: ["Verify Event Monitoring is enabled","Check Setup Audit Trail is logging changes","Test Field Audit Trail on sensitive fields","Verify logs are being exported"],
            ...
            security_tools: {
                splunk: {
                    ...
                    implementation: {
                        steps: ["Deploy Splunk Universal Forwarders on all systems","Configure inputs.conf for log collection","Create indexes for different log types","Configure index retention (7 years for CUI)","Implement RBAC for log access","Set up frozen archive to S3/Azure Blob","Configure summary indexing for long-term analysis","Enable audit logging for Splunk itself"],
...
coldToFrozenDir = /mnt/frozen/cui_audit`,
                        verification: ["Verify forwarders are sending data: index=_internal source=*metrics.log","Check index retention settings","Test frozen archive is working","Verify RBAC restricts log access"],
                ...
                elk: {
                    ...
                    implementation: {
                        steps: ["Deploy Filebeat/Auditbeat on all systems","Configure Logstash pipelines for log parsing","Set up Elasticsearch indexes with ILM policies","Configure index lifecycle (7 years retention)","Implement Elasticsearch security (authentication, RBAC)","Set up snapshot repository for backups","Configure Kibana for log visualization","Enable audit logging for Elasticsearch"],
      ...
      "delete": {
        ...
        "actions": {
          "delete": {}
                        ...
                        verification: ["Verify Beats are sending data: GET _cat/indices","Check ILM policy is applied","Test snapshot repository","Verify RBAC is configured"],
            ...
            small_business: {
                ...
                implementation: {
                    steps: ["Enable CloudTrail/Activity Log/Cloud Audit Logs (free tier)","Use cloud provider's log retention (configure 7 years)","Export logs to low-cost storage (S3 Glacier, Azure Cool Blob)","Use free log analysis tools (CloudWatch Insights, Log Analytics)","Set up basic alerts for critical events","Document log locations and retention policies"],
                    tools: [
            ...
            firewalls: {
                paloalto: {
                    services: ["PAN-OS Logging","Panorama","Cortex Data Lake"],
                    implementation: {"steps":["Configure Traffic, Threat, URL, and Data Filtering logs on all CUI zone policies","Deploy Panorama for centralized log collection and retention","Forward logs to Cortex Data Lake or external SIEM via Syslog/CEF","Enable Enhanced Application Logging for detailed session data","Configure log retention (1 year Panorama, 7 years SIEM)","Enable GlobalProtect activity logging for remote access audit"],"cost_estimate":"$2,000-5,000/year","effort_hours":10}
                }
            },
            xdr_edr: {
                sentinelone: {
                    services: ["Deep Visibility","Singularity Data Lake"],
                    implementation: {"steps":["Enable Deep Visibility for full endpoint telemetry on CUI systems","Configure Singularity Data Lake for long-term EDR data retention","Set up automated threat and activity reports for CUI endpoint groups","Configure SIEM integration for centralized audit log aggregation","Enable Activity Log for admin action audit trail","Schedule automated exports of detection and activity data"],"cost_estimate":"$5-12/endpoint/month","effort_hours":8}
                }
            },
            rmm: {
                ninjaone: {
                    services: ["NinjaOne Activity Log","Monitoring","Scripting"],
                    implementation: {"steps":["Enable NinjaOne Activity Log to track all technician actions on CUI endpoints","Use scripting to collect and forward Windows Event Logs to SIEM","Set up automated scripts to verify audit logging is enabled on CUI endpoints","Configure NinjaOne ticketing to document all CUI system changes","Export activity logs periodically for long-term retention","Monitor for audit log tampering on CUI endpoints"],"cost_estimate":"$3-5/endpoint/month","effort_hours":6}
                }
            },
            vuln_mgmt: {
                tenable: {"services":["Tenable.io","Nessus Compliance"],"implementation":{"steps":["Run compliance audits to verify audit logging is enabled on all CUI systems","Scan for systems with disabled or misconfigured audit policies","Audit log file permissions to ensure logs are protected","Verify log retention configurations meet CUI requirements","Generate audit logging compliance reports"],"cost_estimate":"$30-65/asset/year","effort_hours":6}}
            }
        }
        ...
        "AC.L2-3.1.3": {
            ...
            cloud: {
                aws: {
                    ...
                    implementation: {
                        steps: ["Design VPC architecture with public/private/isolated subnets","Implement Security Groups for stateful firewall rules","Use Network ACLs for subnet-level access control","Enable VPC Flow Logs to monitor traffic patterns","Use AWS PrivateLink for private connectivity to services","Implement Transit Gateway for multi-VPC routing control","Use Resource Access Manager for controlled cross-account sharing","Tag all resources with data classification (CUI, Public, etc.)","Implement S3 bucket policies to control data flow","Use AWS Organizations SCPs to prevent unauthorized data transfers"],
...
resource "aws_flow_log" "cui_vpc_flow_log" {
  ...
  tags = {
    Name = "CUI-VPC-Flow-Logs"
                        ...
                        verification: ["Review VPC Flow Logs for unauthorized traffic patterns","Test Security Group rules block unauthorized access","Verify S3 bucket policies prevent unauthorized data transfers","Check CloudTrail for any policy violations","Use AWS Config to verify resource compliance"],
                ...
                azure: {
                    ...
                    implementation: {
                        steps: ["Design VNet with subnet segmentation for CUI workloads","Implement Network Security Groups (NSGs) for traffic control","Deploy Azure Firewall for centralized network security","Use Private Endpoints for secure service connectivity","Enable NSG Flow Logs for traffic monitoring","Implement Azure Policy to enforce network controls","Use Service Endpoints for Azure service access control","Configure Storage Account network rules to restrict access","Implement Azure Front Door for global traffic management","Use Azure DDoS Protection for CUI resources"],
...
az storage account update \\
  ...
  --name cuistorage \\
  --default-action Deny`,
                        verification: ["Review NSG Flow Logs for traffic patterns","Test NSG rules block unauthorized access","Verify Storage Account network rules","Check Azure Policy compliance","Review Azure Monitor for network alerts"],
                ...
                gcp: {
                    ...
                    implementation: {
                        steps: ["Create VPC with custom subnets for CUI workloads","Implement hierarchical firewall rules","Use VPC Service Controls to create security perimeters","Enable VPC Flow Logs for traffic monitoring","Implement Private Service Connect for Google services","Use Cloud Armor for DDoS protection and WAF","Configure Cloud Storage bucket IAM and ACLs","Implement Organization Policies for network controls","Use Cloud NAT for controlled outbound access","Enable Packet Mirroring for traffic inspection"],
...
gsutil iam ch serviceAccount:cui-app@project.iam.gserviceaccount.com:objectViewer gs://cui-bucket`,
                        verification: ["Review VPC Flow Logs for traffic patterns","Test firewall rules block unauthorized access","Verify VPC Service Controls perimeter","Check Organization Policy compliance","Review Cloud Logging for network events"],
            ...
            containers: {
                kubernetes: {
                    ...
                    implementation: {
                        steps: ["Implement NetworkPolicies for pod-to-pod traffic control","Use namespace isolation for CUI workloads","Deploy service mesh (Istio/Linkerd) for fine-grained traffic control","Configure Ingress controllers with TLS termination","Use Calico or Cilium for advanced network policies","Implement egress gateways to control outbound traffic","Enable mTLS between services","Use network policy logging for audit","Implement DNS policies to control name resolution"],
  ...
  - toEndpoints:
    ...
    - ports:
      ...
      rules:
        ...
        - method: "POST"
          path: "/api/cui/.*"`,
                        verification: ["Test NetworkPolicies block unauthorized pod communication","Verify egress controls prevent data exfiltration","Check service mesh mTLS is enforced","Review network policy logs for violations"],
                        effort_hours: 20
                    }
                }
            },
            saas: {
                microsoft365: {
                    ...
                    implementation: {
                        steps: ["Create DLP policies to prevent CUI data exfiltration","Implement sensitivity labels for CUI classification","Configure Information Barriers to restrict communication","Use Conditional Access to control data flow by location/device","Enable Azure Information Protection for document tracking","Configure Exchange mail flow rules to control CUI emails","Implement SharePoint site permissions for CUI content","Use Microsoft Cloud App Security for shadow IT control","Enable audit logging for all data access"],
...
New-TransportRule -Name "Block CUI External Email" \\
  ...
  -RejectMessageEnhancedStatusCode "5.7.1"`,
                        verification: ["Test DLP policies block unauthorized sharing","Verify sensitivity labels are applied correctly","Check mail flow rules prevent CUI external emails","Review DLP policy matches and incidents"],
            ...
            network: {
                paloalto: {
                    ...
                    implementation: {
                        steps: ["Create security zones for CUI network segments","Implement security policies based on App-ID and User-ID","Configure URL filtering to block unauthorized destinations","Use Data Filtering profiles to prevent CUI exfiltration","Enable SSL/TLS decryption for traffic inspection","Implement zone protection profiles","Configure logging for all security policy actions","Use Panorama for centralized policy management","Implement WildFire for advanced threat prevention"],
...
set rulebase security rules cui-outbound-policy profile-setting profiles data-filtering cui-data-filter`,
                        verification: ["Review traffic logs for policy violations","Test data filtering blocks CUI exfiltration","Verify App-ID correctly identifies applications","Check URL filtering blocks unauthorized sites"],
                ...
                cisco: {
                    ...
                    implementation: {
                        steps: ["Configure security zones for CUI network segments","Implement Access Control Lists (ACLs) for traffic filtering","Use Cisco ISE for identity-based network access control","Configure zone-based firewall policies","Implement TrustSec for software-defined segmentation","Enable NetFlow for traffic monitoring","Configure logging to syslog/SIEM","Implement Cisco Umbrella for DNS security","Use Firepower for next-gen firewall capabilities"],
...
logging host 10.0.100.10 transport udp port 514`,
                        verification: ["Review firewall logs for policy violations","Test ACLs block unauthorized traffic","Verify zone-based policies are enforced","Check NetFlow data for traffic patterns"],
            ...
            small_business: {
                ...
                implementation: {
                    steps: ["Use cloud provider's VPC/VNet with security groups","Implement subnet segmentation for CUI workloads","Configure security group rules to allow only necessary traffic","Enable flow logs (use free tier where available)","Use pfSense or OPNsense for on-premises firewall (free)","Implement basic DLP in Microsoft 365 (E3 license)","Document all network flows and approved paths","Review logs monthly for unauthorized traffic"],
                    tools: [
        ...
        "AC.L2-3.1.4": {
            ...
            cloud: {
                aws: {
                    ...
                    implementation: {
                        steps: ["Define roles with mutually exclusive permissions (e.g., developer vs. production deployer)","Use IAM policies to enforce separation (deny production access to developers)","Implement AWS Organizations SCPs to prevent privilege escalation","Create separate AWS accounts for dev/staging/production","Use IAM permission boundaries to limit maximum permissions","Require multi-person approval for critical actions (AWS Config rules)","Enable CloudTrail to audit all privileged actions","Use AWS Control Tower for guardrails across accounts","Implement break-glass procedures with monitoring","Use AWS SSO with group-based access control"],
...
resource "aws_iam_user" "developer" {
  ...
  tags = {
    Role = "Developer"
                        ...
                        verification: ["Test developers cannot access production resources","Test deployers cannot modify code repositories","Test security auditors cannot modify resources","Review CloudTrail for any privilege escalation attempts","Use AWS IAM Access Analyzer to identify overly permissive policies"],
...
az policy assignment create \\
  ...
  --scope /subscriptions/SUBSCRIPTION_ID`,
                ...
                gcp: {
                    ...
                    implementation: {
                        steps: ["Define custom IAM roles with separation of duties","Use separate GCP projects for dev/staging/production","Implement Organization Policies to enforce constraints","Use IAM Conditions for context-aware access control","Enable Cloud Audit Logs for all privileged actions","Implement VPC Service Controls for data perimeter","Use Resource Manager hierarchy for inheritance","Configure IAM recommender to identify over-privileged accounts","Implement break-glass procedures with monitoring","Use Workload Identity for service-to-service auth"],
...
gcloud projects add-iam-policy-binding prod-project-id \\
  ...
  --condition='expression=request.time < timestamp("2024-12-31T23:59:59Z"),title=Temporary Access,description=Access expires end of year'`,
                        verification: ["Test developers cannot access production project","Test deployers cannot create new resources","Verify IAM conditions enforce time/location restrictions","Review Cloud Audit Logs for privilege escalation","Use IAM recommender to identify excessive permissions"],
            ...
            database: {
                postgresql: {
                    ...
                    implementation: {
                        steps: ["Create separate database roles for different functions","Use schema-level permissions to separate duties","Implement row-level security (RLS) for data access control","Enable pgAudit extension for comprehensive logging","Separate read-only and read-write roles","Implement stored procedures for controlled data modification","Use GRANT/REVOKE to enforce least privilege","Create separate roles for backup/restore operations","Implement connection pooling with role-based routing"],
...
CREATE TRIGGER audit_trigger
  ...
  FOR EACH ROW EXECUTE FUNCTION audit.log_changes();`,
                        verification: ["Test developers cannot access production schema","Test deployers cannot perform DDL operations","Verify row-level security policies are enforced","Review audit logs for all privileged operations","Test that analysts have read-only access"],
                        effort_hours: 16
                    }
                },
                mysql: {
                    ...
                    implementation: {
                        steps: ["Create separate MySQL users for different functions","Use database-level privileges for separation","Enable MySQL Enterprise Audit or MariaDB audit plugin","Implement roles (MySQL 8.0+) for grouped permissions","Separate read-only and read-write users","Use views to restrict data access","Implement stored procedures for controlled operations","Enable binary logging for change tracking"],
...
CREATE TABLE audit_log.user_actions (
  ...
  INDEX idx_timestamp (timestamp)
                        ...
                        verification: ["Test developers cannot access production database","Test deployers cannot perform DDL operations","Verify audit plugin logs all privileged actions","Review role assignments for proper separation","Test that analysts have read-only access"],
                        effort_hours: 14
                    }
                }
            },
            application: {
                nodejs: {
                    ...
                    implementation: {
                        steps: ["Implement role-based access control (RBAC) middleware","Create separate service accounts for different functions","Use environment-specific configuration files","Implement approval workflows for critical operations","Separate read and write API endpoints","Use different database connections for different roles","Implement audit logging for all privileged actions","Use feature flags to control access to sensitive features"],
...
module.exports = { checkPermission, requireApproval };`,
                        verification: ["Test developers cannot deploy to production","Test deployers cannot modify code","Verify approval workflow for critical actions","Review audit logs for all privileged operations","Test role-based access control enforcement"],
                        effort_hours: 24
                    }
                }
            },
            small_business: {
                ...
                implementation: {
                    steps: ["Create separate AWS/Azure/GCP accounts for dev and production","Use built-in IAM roles (no custom development needed)","Implement simple approval process (email/Slack)","Use separate database users with different privileges","Enable cloud provider audit logging (free tier)","Document separation of duties in policy","Use GitHub branch protection for code approval","Implement 2-person rule for production changes"],
                    tools: [
        ...
        "AC.L2-3.1.5": {
            ...
            small_business: {
                approach: "Use cloud provider managed identities and built-in least privilege roles",
        ...
        "AC.L2-3.1.6": {
            ...
            small_business: {
                approach: "Use separate email accounts for admin access, require MFA for admin accounts",
                cost_estimate: "$0",
                effort_hours: 2
            },
            firewalls: {
        ...
        "AC.L2-3.1.7": {
            ...
            small_business: {
                approach: "Remove admin rights from daily user accounts, use separate admin accounts only when needed",
                cost_estimate: "$0",
                effort_hours: 4
            },
            firewalls: {
        ...
        "AC.L2-3.1.8": {
            ...
            small_business: {
                approach: "Enable account lockout in Microsoft 365 (5 attempts, 30 min lockout) and cloud provider settings",
                cost_estimate: "$0",
                effort_hours: 2
            },
            firewalls: {
        ...
        "AC.L2-3.1.9": {
            ...
            implementation: {
                general: {
                    steps: ["Display system use notification before login","Include consent to monitoring statement","Reference applicable CUI rules (32 CFR Part 2002)","Display privacy act statement if collecting PII","Require acknowledgment before access","Include notice in remote access (VPN, RDP)","Post physical security notices at entry points","Update notices when CUI rules change"],
            ...
            small_business: {
                approach: "Add login banner to Microsoft 365, website login pages, and VPN",
                cost_estimate: "$0",
                effort_hours: 2
            },
            firewalls: {
        ...
        "AC.L2-3.1.10": {
            ...
            small_business: {
                approach: "Configure Windows/Mac screen lock via local policy, enforce on mobile devices via Microsoft 365/Google Workspace MDM",
                cost_estimate: "$0",
                effort_hours: 3
            },
            firewalls: {
        ...
        "AC.L2-3.1.11": {
            ...
            small_business: {
                approach: "Configure session timeout in Microsoft 365 (12 hours), web apps (15 min idle)",
                cost_estimate: "$0",
                effort_hours: 2
            },
            firewalls: {
        ...
        "AC.L2-3.1.12": {
            ...
            small_business: {
                approach: "Use Microsoft 365 sign-in logs, Windows RDP logging (Event ID 4624), VPN provider logs",
        ...
        "AC.L2-3.1.13": {
            ...
            small_business: {
                approach: "Use built-in VPN encryption (IKEv2/IPsec), enforce TLS 1.2+ on websites, use encrypted RDP",
                cost_estimate: "$0",
                effort_hours: 3
            },
            firewalls: {
        ...
        "AC.L2-3.1.14": {
            ...
            small_business: {
                approach: "Use VPN as single entry point, remove public IPs from internal systems, use cloud provider bastion services",
        ...
        "AC.L2-3.1.15": {
            ...
            small_business: {
                approach: "Use email approval for privileged access, document all approvals, use cloud provider PIM features",
        ...
        "AC.L2-3.1.16": {
            ...
            small_business: {
                approach: "Use WPA3-Personal with strong passphrase, separate guest network, MAC filtering, cloud RADIUS (JumpCloud, Okta)",
                cost_estimate: "$0-50/month",
                effort_hours: 6
            },
            firewalls: {
        ...
        "AC.L2-3.1.19": {
            ...
            small_business: {
                approach: "Enable BitLocker (Windows Pro+), FileVault (macOS), device encryption (iOS/Android), enforce via MDM",
                cost_estimate: "$0",
                effort_hours: 4
            },
            firewalls: {
        ...
        "AC.L2-3.1.20": {
            ...
            small_business: {
                approach: "Use cloud provider firewall rules, DNS filtering (Cloudflare, Quad9), monitor unusual outbound traffic",
                cost_estimate: "$0-50/month",
                effort_hours: 6
            },
            firewalls: {
        ...
        "AC.L2-3.1.21": {
            ...
            small_business: {
                approach: "Use Group Policy to disable USB storage, whitelist approved devices by serial number, use BitLocker To Go",
                cost_estimate: "$0",
                effort_hours: 4
            },
            firewalls: {
        ...
        "AC.L2-3.1.22": {
            ...
            small_business: {
                approach: "Manual review before publishing, use cloud storage private by default, implement access controls on public websites",
                cost_estimate: "$0-50/month",
                effort_hours: 6
            },
            firewalls: {
        ...
        "AU.L2-3.3.2": {
            ...
            small_business: {
                approach: "Require individual user accounts in Microsoft 365/Google Workspace, disable shared mailboxes for CUI access, enable audit logging",
                cost_estimate: "$0",
                effort_hours: 3
            },
            firewalls: {
        ...
        "AU.L2-3.3.3": {
            ...
            implementation: {
                general: {"steps":["Document current audit log configuration","Review quarterly or after security incidents","Identify gaps in logging coverage","Add logging for new systems/applications","Remove unnecessary logging to reduce noise","Update SIEM correlation rules","Document changes in audit policy","Train SOC team on new log sources"],"effort_hours":8}
            },
            cloud: {
            ...
            small_business: {
                approach: "Review Microsoft 365 audit log configuration quarterly, add new log sources as systems are added",
                cost_estimate: "$0",
                effort_hours: 2
            },
            firewalls: {
        ...
        "AU.L2-3.3.4": {
            ...
            small_business: {
                approach: "Set up email alerts for Microsoft 365 audit log failures, monitor log retention settings",
                cost_estimate: "$0",
                effort_hours: 3
            },
            firewalls: {
        ...
        "AU.L2-3.3.5": {
            ...
            small_business: {
                approach: "Use cloud provider native SIEM (Azure Sentinel, AWS Security Hub), configure basic correlation rules, integrate with email/Slack for alerts",
        ...
        "AU.L2-3.3.6": {
            ...
            small_business: {
                approach: "Use cloud provider log query tools, create basic dashboards, export to Excel for reporting",
                cost_estimate: "$0-50/month",
                effort_hours: 6
            },
            firewalls: {
        ...
        "AU.L2-3.3.7": {
            ...
            small_business: {
                approach: "Use default cloud provider time sync, configure Windows Time service to sync with time.nist.gov",
                cost_estimate: "$0",
                effort_hours: 1
            },
            firewalls: {
        ...
        "AU.L2-3.3.8": {
            ...
            small_business: {
                approach: "Use cloud provider immutable storage, restrict log access to admin accounts only, enable MFA for log access",
        ...
        "AU.L2-3.3.9": {
            ...
            small_business: {
                approach: "Assign audit admin role to 1-2 trusted users, require MFA, document who has audit admin access",
                cost_estimate: "$0",
                effort_hours: 2
            },
            firewalls: {
        ...
        "CM.L2-3.4.1": {
            ...
            small_business: {
                approach: "Document baseline configurations in Excel/SharePoint, use cloud provider native inventory tools, maintain hardware/software list",
                cost_estimate: "$0-20/month",
                effort_hours: 8
            },
            firewalls: {
                paloalto: {
                    services: ["Panorama Templates","Expedition","PAN-OS"],
                    implementation: {"steps":["Use Panorama Templates for standardized firewall configurations","Enable configuration change logging with commit comments","Implement admin role separation for configuration changes","Use Expedition for configuration analysis and best practice compliance","Configure automated configuration backups","Run Best Practice Assessment to detect configuration drift"],"cost_estimate":"$2,000-5,000/year","effort_hours":10}
                }
            },
            xdr_edr: {
                sentinelone: {
                    services: ["Singularity Platform","Application Control"],
                    implementation: {"steps":["Define baseline endpoint security policies for CUI systems","Enable Application Control to whitelist approved software","Monitor for unauthorized software installations via Deep Visibility","Use STAR rules to detect configuration changes on CUI systems","Generate configuration compliance reports from dashboard","Alert on policy changes or agent tampering"],"cost_estimate":"$5-12/endpoint/month","effort_hours":8}
                }
            },
            rmm: {
                ninjaone: {
                    services: ["NinjaOne Patch Management","Scripting","Software Management"],
                    implementation: {"steps":["Create baseline configuration policies for CUI endpoints","Configure automated OS and third-party patching with approval workflows","Use scripting to enforce and audit configuration baselines","Deploy Software Management to control approved application inventory","Set up monitoring for unauthorized configuration changes","Schedule automated compliance scripts to detect baseline drift"],"cost_estimate":"$3-5/endpoint/month","effort_hours":8}
                }
            },
            vuln_mgmt: {
                tenable: {"services":["Tenable.io","Nessus Compliance"],"implementation":{"steps":["Run CIS Benchmark and DISA STIG compliance scans on CUI systems","Create configuration baselines and scan for deviations weekly","Audit installed software against approved lists","Scan for unauthorized services and insecure configurations","Track configuration compliance trends in dashboards","Generate configuration audit reports for assessors"],"cost_estimate":"$30-65/asset/year","effort_hours":10}}
            }
        }
        ...
        "CM.L2-3.4.2": {
            ...
            small_business: {
                approach: "Use cloud provider security baselines, apply Windows security baseline via GPO, document security settings",
                cost_estimate: "$0-50/month",
                effort_hours: 8
            },
            firewalls: {
        ...
        "CM.L2-3.4.3": {
            ...
            small_business: {
                approach: "Use email approval for changes, document changes in Excel/SharePoint, hold monthly change review meetings",
                cost_estimate: "$0",
                effort_hours: 6
            },
            firewalls: {
        ...
        "CM.L2-3.4.5": {
            ...
            small_business: {
                approach: "Restrict production access to 2-3 admins, require MFA, document who has production access",
                cost_estimate: "$0",
                effort_hours: 4
            },
            firewalls: {
        ...
        "CM.L2-3.4.6": {
            ...
            small_business: {
                approach: "Remove unused applications, disable unnecessary Windows services, use minimal cloud VM images",
                cost_estimate: "$0",
                effort_hours: 4
            },
            firewalls: {
        ...
        "CM.L2-3.4.7": {
            ...
            small_business: {
                approach: "Configure firewall to allow only necessary ports (443, 80), use cloud provider security groups, disable unused services",
                cost_estimate: "$0",
                effort_hours: 4
            },
            firewalls: {
        ...
        "CM.L2-3.4.8": {
            ...
            small_business: {
                approach: "Use Windows AppLocker to whitelist approved applications, maintain list of approved software, block admin rights",
                cost_estimate: "$0",
                effort_hours: 8
            },
            firewalls: {
        ...
        "CM.L2-3.4.9": {
            ...
            small_business: {
                approach: "Remove admin rights from users, use Group Policy to block installations, monitor with Windows Event Logs",
                cost_estimate: "$0",
                effort_hours: 6
            },
            firewalls: {
        ...
        "IA.L2-3.5.1": {
            ...
            small_business: {
                approach: "Use individual Microsoft 365/Google Workspace accounts, register devices in MDM, document service accounts",
                cost_estimate: "$0",
                effort_hours: 4
            },
            firewalls: {
                paloalto: {
                    services: ["User-ID","GlobalProtect","SAML/MFA","Certificate Auth"],
                    implementation: {"steps":["Configure User-ID agent for AD integration and user-to-IP mapping","Enable MFA on GlobalProtect VPN using SAML with corporate IdP","Implement certificate-based authentication for device identification","Configure RADIUS/LDAP authentication for firewall admin access with MFA","Implement HIP checks to verify endpoint identity before access","Configure admin role-based authentication with separate credentials"],"cost_estimate":"$3,000-15,000/year","effort_hours":10}
                }
            },
            xdr_edr: {
                sentinelone: {"services":["Singularity Identity","ITDR","Ranger"],"implementation":{"steps":["Enable ITDR for credential attack detection (pass-the-hash, Kerberoasting)","Use Ranger to identify all devices on CUI networks and flag unmanaged endpoints","Configure alerts for suspicious authentication patterns","Integrate with identity providers for correlated visibility","Monitor for unauthorized privilege escalation on CUI systems"],"cost_estimate":"$5-12/endpoint/month","effort_hours":8}}
            },
            rmm: {
                ninjaone: {"services":["NinjaOne Scripting","Monitoring"],"implementation":{"steps":["Use scripting to audit MFA enrollment status across CUI endpoints","Monitor for local account creation or password policy changes","Enforce password complexity and lockout policies via scripting","Audit service account usage and credentials on CUI endpoints","Configure alerts for authentication failures exceeding thresholds"],"cost_estimate":"$3-5/endpoint/month","effort_hours":6}}
            },
            vuln_mgmt: {
                tenable: {"services":["Tenable.io","Nessus","Tenable.ad"],"implementation":{"steps":["Scan for weak or default credentials on CUI systems","Audit password policy compliance via compliance scans","Scan for systems not requiring MFA or using deprecated auth protocols","Use Tenable.ad to assess Active Directory security posture","Identify shared accounts with excessive privileges","Generate authentication compliance reports"],"cost_estimate":"$30-65/asset/year","effort_hours":8}}
            }
        }
        ...
        "IA.L2-3.5.2": {
            ...
            small_business: {
                approach: "Require login for all systems, disable guest accounts, use SSO where possible",
                cost_estimate: "$0",
                effort_hours: 4
            },
            firewalls: {
        ...
        "IA.L2-3.5.3": {
            ...
            small_business: {
                approach: "Enable MFA in Microsoft 365/Google Workspace, use authenticator apps, require MFA for VPN",
                cost_estimate: "$0",
                effort_hours: 3
            },
            firewalls: {
        ...
        "IA.L2-3.5.4": {
            ...
            small_business: {
                approach: "Use modern authentication protocols (OAuth, SAML), enable TLS 1.2+, use time-based MFA",
                cost_estimate: "$0",
                effort_hours: 4
            },
            firewalls: {
        ...
        "IA.L2-3.5.5": {
            ...
            implementation: {
                general: {"steps":["Document identifier reuse policy (minimum 2 years)","Maintain list of retired identifiers","Check against retired list before creating new accounts","Archive user accounts instead of deleting","Use unique employee IDs as basis for usernames","Document exceptions with approval"],"effort_hours":4}
            },
            cloud: {
            ...
            small_business: {
                approach: "Maintain Excel list of retired usernames with retirement dates, check before creating new accounts",
                cost_estimate: "$0",
                effort_hours: 2
            },
            firewalls: {
        ...
        "IA.L2-3.5.6": {
            ...
            small_business: {
                approach: "Review user accounts quarterly, manually disable accounts inactive for 90+ days, document in spreadsheet",
                cost_estimate: "$0",
                effort_hours: 4
            },
            firewalls: {
        ...
        "IA.L2-3.5.7": {
            ...
            small_business: {
                approach: "Configure password policy in Microsoft 365/Google Workspace (14+ chars, complexity, 90-day expiration)",
                cost_estimate: "$0",
                effort_hours: 2
            },
            firewalls: {
        ...
        "IA.L2-3.5.8": {
            ...
            small_business: {
                approach: "Configure password history in Microsoft 365/Google Workspace to prevent reuse of last 24 passwords",
                cost_estimate: "$0",
                effort_hours: 1
            },
            firewalls: {
        ...
        "IA.L2-3.5.9": {
            ...
            small_business: {
                approach: "Enable 'Require password change on first sign-in' in Microsoft 365/Google Workspace when creating accounts",
                cost_estimate: "$0",
                effort_hours: 1
            },
            firewalls: {
        ...
        "IA.L2-3.5.10": {
            ...
            small_business: {
                approach: "Use cloud provider password management (automatic hashing), enforce HTTPS for all web applications",
                cost_estimate: "$0",
                effort_hours: 4
            },
            firewalls: {
        ...
        "IA.L2-3.5.11": {
            ...
            implementation: {
                general: {"steps":["Display asterisks or dots for password input","Use generic error messages ('Invalid username or password')","Don't reveal which field is incorrect","Implement rate limiting to prevent enumeration","Don't display password hints","Mask passwords in logs and error messages","Use CAPTCHA after failed attempts"],"effort_hours":6}
            },
            application: {
            ...
            small_business: {
                approach: "Use default password masking in applications, configure generic error messages in custom apps",
                cost_estimate: "$0",
                effort_hours: 3
            },
            firewalls: {
